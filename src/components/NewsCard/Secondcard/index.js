...
const SecondCard = ({
  ...
  return (
    <div className={`card ${secondStyle.card} `}>
      <a className={secondStyle.link}>@{username}</a>
      <br />
      {dateCreated} <h4 className={secondStyle.title}>{title}</h4>
      {tags.map((tag) => (
        <a className={`${secondStyle.link} ${secondStyle.linkUp}`}>#{tag} </a>
      ))}
      ...
      <div className={secondStyle.downicons}>
        <div className={secondStyle.iconWrapper}>
          <HiThumbUp size={25} color="#757575" />
          <p className={secondStyle.iconCount}>{upVotes}</p>
          <HiThumbDown size={25} color="#757575" />
        ...
        <div className={secondStyle.actions}>
          <div
            className={secondStyle.messageAction}
          >
